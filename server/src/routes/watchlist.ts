...
export default async function watchlistRoutes(app: FastifyInstance) {
  ...
  app.get('/', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      request.log.error({ err, route: 'GET /watchlist', userId: request.userId }, 'Failed to fetch watchlist');
      throw err;
    }
  });
  ...
  app.patch('/:contentId', async (request: FastifyRequest<{ Params: { contentId: string } }>, reply: FastifyReply) => {
    try {
      ...
      request.log.error({ err, route: 'PATCH /watchlist/:contentId', userId: request.userId, params: request.params, body: request.body }, 'Failed to update watchlist entry');
      throw err;
    }
  });
  ...
  app.delete('/:contentId', async (request: FastifyRequest<{ Params: { contentId: string } }>, reply: FastifyReply) => {
    try {
      ...
      request.log.error({ err, route: 'DELETE /watchlist/:contentId', userId: request.userId, params: request.params }, 'Failed to delete watchlist entry');
      throw err;
    }
  });
}
