...
export default async function onboardingRoutes(app: FastifyInstance) {
  ...
  app.get('/genres', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      request.log.error({ err, route: 'GET /onboarding/genres', userId: request.userId }, 'Failed to fetch onboarding genres');
      throw err;
    }
  });
    ...
    try {
      ...
      const seedGenres = seedIds.length > 0
        ? await request.db
            ...
            .where(inArray(contentGenres.contentId, seedIds))
        : [];
      ...
      request.log.error({ err, route: 'GET /onboarding/seeds', userId: request.userId }, 'Failed to fetch onboarding seeds');
      throw err;
    }
  });
  ...
  app.post('/complete', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      request.log.error({ err, route: 'POST /onboarding/complete', userId: request.userId, body: request.body }, 'Failed to complete onboarding');
      throw err;
    }
  });
}
